...
<script type="text/javascript">
    ...
    var fieldsets = getElementsByClass("magicthumb-fieldset");
    var header = null;
    var buttons = null;
    var magicthumbFieldsetId = "";
    for (var i = 0, l = fieldsets.length; i < l; i++) {
        ...
        if (magicthumbFieldsetId != id) {
            magicthumbFieldsetId = id;
        } else {
            header.click();
        }
    }
    initOptionsValidation("magicthumb[desktop][product][template]", "magicthumb[desktop][product][magicscroll]");
    var magicscrollIcon = document.createElement("img");
    magicscrollIcon.id = "magicscroll_icon";
    magicscrollIcon.src = "'.$this->getSkinUrl("magicthumb/magicscroll.png").'";
    ...
    for (var i = 0; i < magictoolboxOptionsLength; i++) {
        ...
        var idMatch = optionName.match(/^magicthumb\[([^\[]+)\]\[([^\[]+)\]\[([^\[]+)\]/i);
            ...
            if (idMatch[2] == "default") {
                ...
                magictoolboxOptions[i].onchange = function() {
                    ...
                    for (var profile in magictoolboxPofiles) {
                        var elements = document.getElementsByName("magicthumb["+thisPlatform+"]["+profile+"]["+thisId+"]");
                        if (elements.length) {
                            if ((elements[0].tagName.toLowerCase() == "select" || elements[0].getAttribute("type") == "text") &&
                                ...
                                var radios = document.getElementsByName("magicthumb["+thisPlatform+"][default]["+thisId+"]");
                                var defaultValue = "";
                                var j;
